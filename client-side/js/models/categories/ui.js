...
export const uiTrZeroSearch = (categorySearch) => {
  return `<tr>
              <td colspan="4" class="text-center align-content-center px-3 fst-italic fw-bold text-capitalize" style="background-color:#f2f2f2">product ${categorySearch} tidak ditemukan....</td>
            </tr>`
}
// when total category row 0 being seaching
...
export const uiBtnPage = (i) => {
  return `<button type = "button" class="category-btn-page ${i === 1 ? 'category-active-page' : ''}" >
                  ${i}
          </button>`
}
// Function to update active page button
export const uiActivePageButton = (categoryPageNumber, categoryBtnPage) => {
  const categoryBtnPageActive = document.getElementsByClassName("category-active-page");
