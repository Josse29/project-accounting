...
ipcMain.on("pdf:product", (event) => {
  ...
  ipcMain.handle("convert-to-pdf", async (event, pdfName) => {
    const pdfPath = path.join(__dirname, "output", `${pdfName}.pdf`);
    // Mengambil isi dari halaman sebagai PDF
    ...
    return pdfPath; // Kembalikan path PDF untuk konfirmasi
  });
});
// 2. persediaan
let persediaanPdf;
